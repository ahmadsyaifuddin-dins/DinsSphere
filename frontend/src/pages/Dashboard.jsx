...
export const GoogleGeminiEffect = ({
  ...
  const transition = { duration: 0.8, ease: "easeInOut" }; // Added proper animation duration

  return (
    <div className={cn("relative", className)}>
      <div className="flex flex-col items-center justify-center pt-20">
        <p className="text-xl md:text-6xl font-bold pb-4 text-center bg-clip-text text-transparent bg-gradient-to-r from-blue-400 via-purple-500 to-pink-500">
          {title || `DinsSphere InterConnected`}
        </p>
        <p className="text-sm md:text-lg font-normal text-center text-neutral-300 mt-2 max-w-lg mx-auto">
          {description || "Vibecheck your digital ecosystem with DinsSphere 🔥"}
        </p>
        <button className="font-medium bg-gradient-to-r from-blue-500 to-purple-600 rounded-full px-6 py-2 mt-8 z-30 text-white text-sm hover:shadow-lg hover:shadow-blue-500/20 transition-all duration-200">
      ...
      >
        ...
        <defs>
          ...
          <linearGradient id="gradient" x1="0%" y1="0%" x2="100%" y2="0%">
            <stop offset="0%" stopColor="#4FABFF" stopOpacity="0.3" />
            <stop offset="50%" stopColor="#B1C5FF" stopOpacity="0.3" />
            <stop offset="100%" stopColor="#FFB7C5" stopOpacity="0.3" />
          </linearGradient>
        </defs>
...
// Dashboard Component yang mengintegrasikan GoogleGeminiEffect
function Dashboard() {
  // State for path animations
  ...
  useEffect(() => {
    ...
    // Optional: Add scroll listener for interactive animations
    const handleScroll = () => {
      ...
      // Only animate if user has scrolled down a bit
  ...
  return (
    <div className="relative w-full min-h-screen bg-slate-950 text-white">
      {/* Hero section with GoogleGeminiEffect */}
      ...
      {/* Dashboard content */}
      {/* <main className="relative z-10 container mx-auto px-4 py-16 pb-32">
        <h2 className="text-3xl font-bold mb-8">Dashboard Overview</h2>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {["Active Users", "Total Connections", "System Health"].map(
            (title, index) => (
              <div
                key={index}
              ...
              >
                ...
                <p className="text-3xl font-bold mt-2">{(index + 1) * 1234}</p>
                <p className="text-green-400 text-sm mt-2">
                  +{5 + index}% from last period
                </p>
              </div>
            )
          )}
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-sm text-neutral-400">
                    Today at {10 + index}:{index * 15 || "00"} AM
                  </p>
                </div>
              </div>
            ))}
      ...
      {/* Footer */}
      {/* <footer className="relative z-10 border-t border-white/10 backdrop-blur-md bg-black/50 py-6">
        <div className="container mx-auto px-4 text-center text-sm text-neutral-400">
          <p>&copy; 2025 DinsSphere. All rights reserved.</p>
          <p className="mt-2">Powered by the latest tech innovations</p>
        </div>
      </footer> */}
    </div>
  );
