...
// GoogleGeminiEffect Component with enhanced visuals
export const GoogleGeminiEffect = ({
  ...
  return (
    <div className={cn("relative overflow-hidden w-full h-screen", className)}>
      ...
      {/* Improved SVG with better integration */}
      <svg
        width="100%"
        height="700"
      ...
      >
        {/* Improved path styling with glow effects */}
        <motion.path
        ...
        {/* Interactive highlight spots */}
        {[400, 600, 800, 1000, 1200].map((x, i) => (
          <motion.circle
            key={i}
            cx={x}
            cy={480 - i * 20}
      ...
      {/* Interactive orbs floating in background */}
...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    // Add scroll listener for interactive animations
    const handleScroll = () => {
  ...
  return (
    <div className="relative w-full min-h-screen bg-slate-950 text-white overflow-hidden">
      {/* Hero section with enhanced GoogleGeminiEffect */}
      ...
      {/* Your existing commented code for Dashboard content remains the same */}
    </div>
  );
}
