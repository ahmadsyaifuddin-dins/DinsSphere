        ...
        <div className="max-w-xs overflow-hidden text-xs">
          {keys.map(key => (
            <div key={key} className="truncate">
              ...
              <span className="text-gray-300">{JSON.stringify(parsed[key]).substring(0, 30)}</span>
            </div>
          ))}
        </div>
      );
      ...
      <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-8 bg-gray-800 rounded-xl p-6 border border-gray-700 shadow-lg">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-gray-400">@{userData.username} • {userData.email}</p>
                <div className="mt-2">
                  <span className={`px-3 py-1 text-xs rounded-full ${userData.isActive ? 'bg-green-600' : 'bg-red-600'}`}>
        ...
        {isLoading ? (
          ...
          <div className="bg-gray-800 rounded-lg p-8 text-center border border-gray-700">
            ...
            <p className="text-xl text-gray-300">No activities found for this user.</p>
            <p className="text-gray-400 mt-2">This user hasn't performed any tracked actions yet.</p>
          </div>
        ) : (
          <>
              ...
              <table className="min-w-full divide-y divide-gray-700">
                ...
                <tbody className="divide-y divide-gray-700">
                  {activities.map((activity, index) => (
                    <tr key={activity._id} className={index % 2 === 0 ? "bg-gray-800" : "bg-gray-750"}>
                      <td className="py-3 px-4 whitespace-nowrap text-sm">
                        <div className="font-medium">{new Date(activity.createdAt).toLocaleDateString()}</div>
                        <div className="text-gray-400 text-xs">{new Date(activity.createdAt).toLocaleTimeString()}</div>
                      </td>
                      <td className="py-3 px-4 whitespace-nowrap">
                        <span className={`px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${getActivityTypeColor(activity.type)} text-white`}>
                          {activity.type}
                        </span>
                      </td>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {Object.entries(
                  ...
                  <div key={type} className="bg-gray-700 rounded-lg p-4 text-center">
                    <div className={`inline-block px-3 py-1 rounded-full ${getActivityTypeColor(type)} text-white text-xs mb-2`}>
                      {type}
                    </div>
